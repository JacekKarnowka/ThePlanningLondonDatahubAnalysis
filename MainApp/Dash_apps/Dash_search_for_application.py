...
def total_no_res_units_by_date(df):
    ...
    total_res_units = Views_Graph_Objects.line_plot(
        "Date", "Number of residential units", True, True, 450
    )
    ...
    for borough in boroughs:
        ...
        x_values = sorted(
            list(total_df[total_df["Borough"] == borough]["decision_date"])
        )

        # For each unique borough get 'proposed residential units' value
        tmp = list(
            float(i)
            for i in total_df[total_df["Borough"] == borough][
        ...
        total_res_units.add_trace_to_fig(
            x_values,
            y_sum,
            borough,
        )
...
def dash_app(df_tenure_data):
    ...
    density_NO_units_bar_chart = Views_Graph_Objects.density_and_NO_units_bar_chart(
        df_tenure_data
    )

    density_affordable_types_bar_chart = (
        Views_Graph_Objects.density_and_affordable_types_bar_chart(df_tenure_data)
    )

    density_NO_storeys_bar_chart = Views_Graph_Objects.density_and_NO_storeys_bar_chart(
        df_tenure_data
    )
                ...
                [
                    html.Div(
                        [
                            html.Div(
                                [
                                    html.H2(
                                        children="Choose created project for comparison",
                                        id="dropdown_text",
                                    )
                                ],
                    ...
                    html.Div(
                        [
                            ...
                            dcc.Slider(
                                ...
                                id="map_loc_slider",
                                tooltip={
                                    "allways_visible": True,
                                    "placement": "bottom",
                                },
                                dots=True,
                            ),
                            dcc.Graph(id="map_location", figure=all_app_plot),
                        ],
                        className="block full",
                    ),
                    ...
                    html.Div(
                        [
                            ...
                            dcc.Graph(id="res_units_by_date", figure=res_units_by_date),
                        ],
                        className="block full",
                    ),
                                ...
                                [
                                    html.Div(
                                        ...
                                        children=dash_table.DataTable(
                                            data=status_table.to_dict("rows"),
                                            columns=[
                                                {"name": i, "id": i}
                                                for i in status_table.columns
                                            ],
                                            id="status_table",
                                            style_data={
                                                "text-align": "left",
                    ...
                    html.Div(
                        [
                            html.Div(
                                [
                                    html.H2(
                                        children="Density and number of units bar chart"
                                    )
                                ],
                                className="block-title",
                            ),
    ...
    def callback_radius(value, active_cell, table_data):
        ...
        if active_cell != None:
            ...
            all_apps_loc = Views_Graph_Objects.map_locations_all_appls(
                df_tenure_data3, size=11
            )
            all_app_plot = all_apps_loc.print_plot()
    ...
    def callback_radius(value, active_cell, table_data):
        ...
        if active_cell != None:
            ...
            df_tenure_data_updated = df_tenure_data2[
                df_tenure_data2["Status"] == status
            ]
    ...
    @app.callback(
        Output("status_table_div", "children"), Input("map_loc_slider", "value")
    )
    ...
    @app.callback(
        Output("status_pie_chart", "figure"), Input("map_loc_slider", "value")
    )
    def change_table_data(slider_value):

        df_tenure_data_updated = comparsion_form_radius_filter(
            df_tenure_data, slider_value
        )
    ...
    def dwell_density_total_number_units(value, active_cell, table_data):
        ...
        if active_cell != None:
            ...
            df_tenure_data_updated = df_tenure_data2.loc[
                (df_tenure_data2["Status"] == status) | (df_tenure_data2["New"] == 1)
            ]

            updated = Views_Graph_Objects.density_and_NO_units_bar_chart(
                df_tenure_data_updated
            )
    ...
    def dwell_density_aff_percentage(value, active_cell, table_data):
        ...
        if active_cell != None:
            ...
            updated = Views_Graph_Objects.density_and_affordable_types_bar_chart(
                df_tenure_data_updated
            )
    ...
    def dwell_density_and_NO_storeys(value, active_cell, table_data):
        ...
        if active_cell != None:
            ...
            updated = Views_Graph_Objects.density_and_NO_storeys_bar_chart(
                df_tenure_data_updated
            )
    ...
    @app.callback(
        ...
        [Input("projects_dropdown", "value"), Input("map_loc_slider", "value")],
    )
    def dropdown(value, slider_value):
        ...
        df_tenure_data = pd.concat(
            [df_tenure_data, df_projects], ignore_index=True, sort=False
        )
