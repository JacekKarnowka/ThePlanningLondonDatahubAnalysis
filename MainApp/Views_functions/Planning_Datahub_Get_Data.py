...
def get_data(site_area_filter, units_filter, date_from, date_to, status = "*", lpa_name = "*"):
    ...
    ### FEATURES
    source_ = []
    source_ = source_ + ["lpa_name", "ward", "street_name", "postcode", "polygon", "centroid"]
    source_ = source_ + ["lpa_app_no", "site_name"]

    source_ = source_ + ["appeal_decision", "appeal_status", "application_type", "application_type_full", "decision", "decision_agency", "decision_process", "development_type", "status"]
    ...
    try:
        sitearea_filter = 'application_details.residential_details.site_area:[{} TO {}]'.format(site_area_range[0], site_area_range[1])
        url += ' AND {}'.format(sitearea_filter)
    except IndexError:
        sitearea_filter = "*"

    try:
        units_filter = 'application_details.residential_details.total_no_proposed_residential_units:[{} TO {}]'.format(units_filter_range[0], units_filter_range[1])
        url += ' AND {}'.format(units_filter)
    except IndexError:
        units_filter = "*"

    decision_date_filter = 'decision_date:[{} TO {}]'.format(date_from_range, date_to_range)
