...
def get_dictionary_residential_units_data(tmp):
    ...
    tmp_dict2["unit_development_type"] = dict(
        df2["unit_development_type"].value_counts()
    )
...
def get_more_residential_units_data(df):
    ...
    for i in values:
        df["app_det_left_{}".format(i)] = df["source"].apply(
            lambda x: float(x["application_details"]["residential_details"][i])
            if x["application_details"]["residential_details"][i] != None
            else 0
        )
...
def get_floorspace_details(df):
    ...
    floorspace_details = [
        ...
        "gia_gained",
        "gia_lost",
        "superseded_by_lpa_app_no",
        "superseded_date",
        "use_class",
    ]

    for det in floorspace_details:
        df["floorspace_detail_{}".format(det)] = df["source"].apply(
            lambda x: x["application_details"]["existing_proposed_floorspace_details"][
                0
            ][det]
            if ("existing_proposed_floorspace_details" in x["application_details"])
            and (
                len(x["application_details"]["existing_proposed_floorspace_details"])
                > 0
            )
            and (
                det
                in x["application_details"]["existing_proposed_floorspace_details"][0]
            )
            else "nan"
        )
    return df
...
def get_units_info_details(df):
    ...
    for det in units_info_details:
        df["app_det_res_units_{}".format(det)] = df["Units_info"].apply(
            lambda x: x[det] if det in x else "nan"
        )
    ...
    for new_col in development_type_list:
        df["Development type: {}".format(new_col)] = df[
            "app_det_res_units_unit_development_type"
        ].apply(lambda x: x[new_col] if new_col in x else 0)
...
def get_parking_details(df):
    ...
    parking_details_list = [
        ...
        "net_other",
        "net_resi_offstreet",
        "no_existing_bluebadge",
        "no_existing_bus",
        "no_existing_car",
        "no_existing_carclub",
        "no_existing_cycle",
        "no_existing_lgv",
        "no_existing_motorcycle",
        "no_existing_other",
        "no_existing_resi_offstreet",
        "no_proposed_bluebadge",
        "no_proposed_bus",
        "no_proposed_car",
        "no_proposed_carclub",
        "no_proposed_cycle",
        "no_proposed_lgv",
        "no_proposed_motorcycle",
        "no_proposed_other",
        "no_proposed_resi_offstreet",
    ]

    for details in parking_details_list:
        try:
            df["parking_{}".format(details)] = df["Parking"].apply(
                lambda x: x[details] if (details in x) and (x[details] != "nan") else 0
            )
        except TypeError:
            print()
...
def cleaning_data(df):
    ...
    new_df["Units_info"] = new_df["source"].apply(
        lambda x: get_dictionary_residential_units_data(x)
    )

    new_df["Site_area"] = new_df["source"].apply(
        lambda x: x["application_details"]["site_area"]
        if ("site_area" in x["application_details"])
        else "nan"
    )

    new_df["NO storeys"] = new_df["source"].apply(
        lambda x: x["application_details"]["building_details"][0]["no_storeys"]
        if "building_details" in x["application_details"]
        and x["application_details"]["building_details"] != None
        and len(x["application_details"]["building_details"]) > 0
        else "NAN"
    )
    ...
    new_df["app_det_left_affordable_percentage"] = new_df[
        "app_det_left_affordable_percentage"
    ].apply(lambda x: str(round(float(x), 2)) + " %")
    ...
    new_df["app_det_left_dwelling_density"] = new_df[
        "app_det_left_dwelling_density"
    ].apply(lambda x: str(round(float(x), 2)))

    new_df.rename(
        columns={
            "app_det_left_total_no_proposed_residential_units": "NO proposed residential units",
            "app_det_left_affordable_percentage": "Affordable Percentage",
        },
        inplace=True,
    )
