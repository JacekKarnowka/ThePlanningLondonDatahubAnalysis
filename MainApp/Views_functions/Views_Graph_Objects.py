...
class line_plot:
    def __init__(
        self, x_title, y_title, legend_bool, xaxis_title_bool, height, app_id=None
    ):
        self.app_id = app_id
    ...
    def add_trace_to_fig(self, x_values, y_values, name):
        ...
        self.fig.add_trace(go.Scatter(x=self.x_values, y=self.y_values, name=self.name))
    ...
    def print_plot(self):
        ...
        self.fig.update_layout(
            ...
            yaxis=dict(
                title=self.y_title, showgrid=True, showline=True, showticklabels=True
            ),
            xaxis=dict(title=self.x_title, showticklabels=self.xaxis_title_bool),
            showlegend=self.legend_bool,
            font={"color": "gray", "family": "sans-serif", "size": 12},
            margin={
                "b": 0,
                "l": 0,
                "r": 0,
...
class status_pie_chart:
    ...
    def create_plot(self):
        ...
        fig.add_trace(go.Pie(values=self.plot_1["count"], labels=self.plot_1["Status"]))
...
class bar_plot_details:
    ...
    def print_plot(self):
        ...
        fig.update_layout(
            autotypenumbers="convert types",
            yaxis=dict(
                title=self.y_name, showgrid=True, showline=True, showticklabels=True
            ),
...
def tenure_pie_chart(app_id):
    ...
    fig_4.update_layout(
        font={"color": "gray", "family": "sans-serif", "size": 12},
        showlegend=True,
        legend=dict(orientation="v", y=0, x=0),
        margin={
            "b": 0,
            "l": 0,
            "r": 0,
...
class map_locations_all_appls:
    ...
    def print_plot(self):
        ...
        if self.color == None:
            marker = {"color": self.df.color, "size": self.df["size"], "opacity": 0.9}
        else:
            marker = {"color": self.color, "size": self.df["size"], "opacity": 0.9}
        ...
        fig.add_trace(
            go.Scattermapbox(
                ...
                marker=go.scattermapbox.Marker(marker),
            )
        )
...
def density_and_NO_units_bar_chart(df):
    ...
    for app in app_list:
        fig.append_trace(
            go.Bar(
                ...
                marker={"color": df[df["App_ID"] == app]["color"], "showscale": False},
                hoverinfo="text",
                hovertext=df[df["App_ID"] == app]["Dwelling_density"],
                name=df.loc[df["App_ID"] == app, ["Status"]].values[0][0],
                showlegend=True
                if df.loc[df["App_ID"] == app, ["Status"]].values[0][0]
                not in status_set
                else False,
            ),
            1,
    ...
    fig.update_layout(
        ...
        yaxis=dict(
            title="Dwelling density", showgrid=True, showline=True, showticklabels=True
        ),
        yaxis2=dict(
        ...
        title=dict(
            text="Dwellig density and total number of units",
            y=0.95,
            x=0.5,
            xanchor="center",
            yanchor="top",
...
def density_and_affordable_types_bar_chart(df):
    ...
    fig.add_trace(
        go.Bar(
            ...
            marker={"color": "rgb(155,153,155)", "showscale": False},
            showlegend=False,
            hoverinfo="text",
    ...
    fig.update_layout(
        ...
        title=dict(
            text="Dwellig density and affordable percentage (with specified type)",
            y=0.95,
            x=0.5,
            xanchor="center",
            yanchor="top",
        ...
        yaxis1=dict(
            title="Number of units (affordable types)",
            showgrid=True,
            showline=True,
            showticklabels=True,
        ),
...
def density_and_NO_storeys_bar_chart(df):
    ...
    try:
        df["NO_storeys"] = df["NO_storeys"].apply(
            lambda x: float(x) if x != "nan" else 0
        )
    except TypeError:
        df["NO_storeys"] = 0
    ...
    fig.update_layout(
        ...
        showlegend=False,
        yaxis=dict(
            title="Number of storeys", showgrid=True, showline=True, showticklabels=True
        ),
        yaxis2=dict(
            title="Dwelling density", showgrid=True, showline=True, showticklabels=True
        ),
        title=dict(
            text="Number of storeys and dwelling density",
            y=0.95,
            x=0.5,
            xanchor="center",
            yanchor="top",
