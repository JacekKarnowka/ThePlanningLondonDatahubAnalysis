...
def get_status_color(df):
    df["color"] = df["Status"].apply(
        lambda x: "rgb(124,98,78)"
        if x == "Allowed"
        else "rgb(226,207,190)"
        if x == "Approved"
        else "rgb(109,148,198)"
        if x == "Commenced"
        else "rgb(3,100,110)"
        if x == "Completed"
        else "rgb(241,234,226)"
        if x == "Lapsed"
        else "rgb(217,50,34)"
        if x == "Submited"
...
class search_for_application(View):
    ...
    def get(self, request):
        ...
        projects_database = list(
            created_projects.objects.values_list("App_ID", flat=True)
        )
        ...
        return render(
            request,
            "front/search_for_application.html",
            {"projects_database": projects_database, "tenure_data": tenure_data},
        )
    ...
    def post(self, request):
        ...
        if "search_filter" in request.POST:
            ...
            if date_from == "" or date_to == "":
                ...
                df_tenure_data = pd.read_sql_query(
                    "SELECT * from  MainApp_allapps", cnx
                )
                cnx.commit()
                cnx.close()
                ...
                date_from_range = pd.Timestamp(
                    day=int(date_from_m_d_y[1]),
                    month=int(date_from_m_d_y[0]),
                    year=int(date_from_m_d_y[2]),
                )

                date_to_range = pd.Timestamp(
                    day=int(date_to_m_d_y[1]),
                ...
                cnx = sqlite3.connect("db.sqlite3")
                df_tenure_data = pd.read_sql_query(
                    "SELECT * from  MainApp_allapps WHERE decision_date BETWEEN '{}'  AND '{}'".format(
                ...
                print("Read from Db do DF : {}".format(time.time() - start_time))
            # Save to database
            Planning_Datahub_Save_to_Database.save_to_database(
                TenureData, df_tenure_data
            )
            ...
            for i in range(18, 19):
                print(
                    "Searching for application in range: 01/01/20{}  TO  12/31/20{}".format(
                        i, i
                    )
                )
                ...
                df_raw_data = Planning_Datahub_Get_Data.get_data(
                    site_area, units_filter, date_from, date_to, status, lpa_name
                )
                df_cleaned_data = (
                    Planning_Datahub_Get_Res_Units_Data_Cleaning.cleaning_data(
                        df_raw_data
                    )
                )
                ...
                df = Planning_Datahub_geometry_functions.get_centroid_data(
                    df_cleaned_data
                )
                ...
                df_tenure_data["decision_date"] = df_tenure_data["decision_date"].apply(
...
class application_details(View):
    def get(self, request, word):
        ...
        parking_fields = [
            k
            for k in TenureData.objects.filter(App_ID=word).values()[0]
            if "parking" in k
        ]
        parking_dict = dict()
        ...
        app_map_location = Views_Graph_Objects.map_locations_all_appls(
            df[df["App_ID"] == word],
            13,
            "rgb(255, 0, 0)",
        ...
        app_map_location_plot = plot(
            figure_or_data=app_map_location_plot, output_type="div"
        )
        ...
        if df[df["App_ID"] == word]["Affordable_Percentage"].iloc[0] != "0.0 %":
            ...
            affordable_details = Views_Graph_Objects.bar_plot_details(
                word, affortable_units_details, "Affordable types"
            )
        ...
        units_types = Views_Graph_Objects.bar_plot_details(
            word, units_types_details, "Units type"
        )
        ...
        try:
            ...
            GIA_data_plot = Views_Graph_Objects.line_plot(
                "Change", "GIA values", False, False, 275, word
            )
            GIA_data_plot.add_trace_to_fig(x_values, y_values, "GIA")
            line_GIA_plot = GIA_data_plot.print_plot()
            line_GIA_plot = plot(figure_or_data=line_GIA_plot, output_type="div")
